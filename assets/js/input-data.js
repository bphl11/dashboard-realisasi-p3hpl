...
function opt(el,vals,ph,on){
  el.innerHTML='<option value="">'+ph+'</option>';
  [...new Set(vals.filter(hasValue).map(clean))]
...
function reset(n){
  ["Pilih Sub Output","Pilih Komponen","Pilih Sub Komponen","Pilih Akun Belanja","Pilih Item Akun","Pilih Rincian Item"]
    .slice(n)
    .forEach((p,i)=>opt(els[n+i],[],p,false));
}

function filtered(){
  return rows.filter(r=>
    (!so.value||clean(r.subOutput)===so.value)&&
    (!k.value||clean(r.komponen)===k.value)&&
    (!sk.value||clean(r.subKomponen)===sk.value)&&
    (!a.value||clean(r.akun)===a.value)&&
    (!ia.value||clean(r.itemAkun)===ia.value)
  );
}
...
ri.onchange=()=>{
  const x=filtered().filter(r=>clean(r.rincianItem)===ri.value);

  if(x.length===1){
    const r=x[0];
    const months=Object.values(r.bulanan||{}).reduce((s,v)=>s+(Number(v)||0),0);
    ...
    const fields=[
      ["Sub Output",r.subOutput],
      ["Komponen",r.komponen],
      ["Sub Komponen",r.subKomponen],
      ["Akun Belanja",r.akun],
      ["Item Akun",r.itemAkun],
      ["Rincian Item",r.rincianItem],
    ...
    out.innerHTML=fields.map(z=>
      '<div class="result-item"><div class="result-label">'+z[0]+
      '</div><div class="result-value">'+String(z[1]??"")+
      '</div></div>'
    ).join("");
  }else{
    status.className="verify-badge "+(x.length?"warn":"error");
    status.innerHTML='<i class="bi bi-exclamation-triangle-fill"></i> '+x.length+" record ditemukan";
    msg.textContent=x.length?"Kombinasi belum unik.":"Record tidak ditemukan.";
    out.innerHTML="";
  }
};
...
try{
  ...
  // Jangan membatasi hanya record isRincian.
  // Semua record DATA_APLIKASI yang memiliki identitas hierarki
  // tetap dipakai agar pilihan berjenjang tidak hilang.
  rows=parsed.filter(r=>
    hasValue(r.subOutput)||
    hasValue(r.komponen)||
    hasValue(r.subKomponen)||
    hasValue(r.akun)||
    hasValue(r.itemAkun)||
    hasValue(r.rincianItem)
  );
